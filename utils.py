...
# 根据元素属性判断元素是否存在公用的方法
def is_el_by_attri_exist(driver, attr_name, attr_value):
    # 定位元素的xpath表达式
    ...
    try:
        ...
        NoSuchElementException("找不到元素为{}的元素对象".format(attr_name, attr_value))
        return False
...
def check_channel_option(driver, channel_name, option_name):
    ...
    for option_element in channel_option:
        # 判断当前的元素文本信息是不是想要选的文本信息
        if option_element.text == option_name:
            # 如果是则跳出，则点击，把标识改为True
            option_element.click()
            is_suc = True
            break
        # 如果不相等，鼠标悬浮到当前遍历的元素对象上并按向下的按键
        else:
            # 创建鼠标对象
